...
const Profile = ({ onUpdateProfiles }) => {
  ...
  const fetchBreeds = (speciesId) => {
    axios
      .get(`http://localhost:3000/api/breeds?speciesId=${speciesId}`)
      .then((response) => {
    ...
    const fetchPetProfiles = async () => {
      ...
      try {
        ...
        const response = await axios.post(
          `http://localhost:3000/pet/${userContext.userId}`
        ); // <= change!!!
  ...
  const handleImageChange = async (e) => {
    console.debug('Files selected:', e.target.files);
    ...
    if (e.target.files.length > 0) {
      ...
      file
        .arrayBuffer()
        .then((arr) => {
          console.log('PROFILE PIC DATA: ', arr);
          axios
            .post(`http://localhost:3000/api/postProfilePicture`, {
              arr: arr,
              key: userContext.userId,
            })
            .then((res) => {
              console.log('PROFILE PIC SENT: ', arr);
            })
            .catch((err) => {
              console.log(err);
            });
      ...
      //   console.log('PROFILE PIC DATA: ', reader.result);
      // });
